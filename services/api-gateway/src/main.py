...
async def _run_analysis_background(headline: str) -> None:
    """Background task: LLM analysis + agent-driven simulation, stores to Redis."""
    ...
    model = os.getenv("PRIMARY_LLM_MODEL", "qwen-plus")

    def _sync_llm(system_prompt: str, user_msg: str) -> str:
    ...
    async def llm_call(system_prompt: str, user_msg: str) -> str:
        return await asyncio.get_event_loop().run_in_executor(
            None, _sync_llm, system_prompt, user_msg,
        )
    ...
    r = aioredis.from_url(REDIS_URL, decode_responses=True)
    try:
        await r.set("event_analysis:status", "running")
    finally:
        await r.aclose()

    try:
        # Step 1: LLM analyzes the event
        logger.info("Event analysis started: %s", headline[:80])
        analysis = await analyze_event(headline, llm_call, timeout=60)
        if analysis is None:
            r = aioredis.from_url(REDIS_URL, decode_responses=True)
            await r.set("event_analysis:status", "error: LLM analysis failed")
            await r.aclose()
            return

        logger.info("LLM returned %d impacts, starting simulation...", len(analysis.get("impacts", [])))

        # Step 2: Run per-asset agent-driven simulation
        # Use fewer ticks and agents to stay within API rate limits
        ...
        r = aioredis.from_url(REDIS_URL, decode_responses=True)
        try:
            await r.set("event_analysis:latest", json.dumps(analysis, default=str))
            await r.set("event_analysis:status", "complete")
            await r.lpush("event_analysis:history", json.dumps(analysis, default=str))
            await r.ltrim("event_analysis:history", 0, 19)
        finally:
            await r.aclose()

        logger.info("Event analysis complete: %d assets simulated", len(sim_results))

    except Exception as exc:
        logger.error("Event analysis failed: %s", exc)
        r = aioredis.from_url(REDIS_URL, decode_responses=True)
        await r.set("event_analysis:status", f"error: {exc}")
        await r.aclose()


@app.post("/api/event-analysis/run")
async def run_event_analysis(req: EventAnalysisRequest):
    """Start LLM event analysis + agent-driven multi-asset simulation in background.

    Returns immediately. Frontend polls /api/event-analysis/latest for results.
    """
    global _analysis_task

    api_key = os.getenv("DASHSCOPE_API_KEY", "")
    if not api_key:
        raise HTTPException(400, "DASHSCOPE_API_KEY not set")

    # Cancel any existing analysis
    if _analysis_task and not _analysis_task.done():
        _analysis_task.cancel()

    _analysis_task = asyncio.create_task(_run_analysis_background(req.headline))

    return {"status": "started", "message": "Simulation running in background"}


@app.get("/api/event-analysis/status")
async def get_analysis_status():
    """Check if a background analysis is still running."""
    import redis.asyncio as aioredis
    r = aioredis.from_url(REDIS_URL, decode_responses=True)
    try:
        status = await r.get("event_analysis:status") or "idle"
    finally:
        await r.aclose()
